  ...
  <div class="flex h-screen overflow-hidden bg-brand-dark">
    ...
    >
      ...
      <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        <router-link v-if="auth.user?.tenant?.active_modules?.finances" to="/billing" custom v-slot="{ isActive, navigate }">
          <NavItem icon="credit-card" label="Finanzas" :active="isActive" @click="navigate(); isMobileClose()" />
        </router-link>
      ...
      >
        <div class="flex items-center gap-4">
          ...
          <div>
            <h2 class="text-base font-semibold text-slate-100">
              <slot name="title">Panel de control</slot>
            </h2>
            <p class="text-xs hidden sm:block text-slate-400">
              <slot name="subtitle">Bienvenido de vuelta, {{ auth.user?.name ?? auth.user?.email ?? '—' }}</slot>
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="hidden sm:block">
            ...
            <p class="text-xs leading-tight truncate max-w-[120px] text-slate-400">{{ auth.user?.email ?? '—' }}</p>
          </div>
        </div>
      </header>
