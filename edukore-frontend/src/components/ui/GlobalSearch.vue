  ...
  <div class="relative w-full max-w-md">
    <div class="relative">
      ...
      <input
        ...
        class="block w-full pl-10 pr-3 py-2 border border-brand-border rounded-xl leading-5 bg-brand-surface text-slate-300 placeholder-slate-500 focus:outline-none focus:bg-white/10 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm transition-colors"
      />
    ...
    <div v-if="showDropdown && results.length > 0" class="absolute z-50 mt-1 w-full bg-slate-800 border border-brand-border rounded-xl shadow-lg overflow-hidden">
      <ul class="max-h-60 overflow-y-auto py-1 text-sm text-slate-300">
        ...
        <li 
          ...
          class="px-4 py-2 hover:bg-primary-600 hover:text-white cursor-pointer transition-colors"
        >
