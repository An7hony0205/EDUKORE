    ...
    <div class="max-w-7xl mx-auto space-y-6">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p class="text-slate-400 text-sm mt-1">Gestiona los perfiles y matrículas del alumnado.</p>
        </div>
        <router-link to="/students/new" class="bg-primary-600 hover:bg-primary-500 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
      ...
      <div class="bg-brand-surface border border-brand-border rounded-2xl overflow-hidden">
        <div class="p-4 border-b border-brand-border flex gap-4">
          <input 
            v-model="search" 
            @input="fetchStudents(1)"
            type="text" 
            placeholder="Buscar por nombre o correo..." 
            class="bg-white/5 border border-brand-border rounded-lg px-4 py-2 text-white text-sm w-full max-w-sm focus:outline-none focus:border-primary-500"
          >
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm text-slate-300">
            ...
            <tbody>
              <tr v-if="loading" class="border-b border-brand-border">
                <td colspan="4" class="px-6 py-8 text-center text-slate-500">Cargando directorio...</td>
              </tr>
              <tr v-else-if="students.length === 0" class="border-b border-brand-border">
                <td colspan="4" class="px-6 py-8 text-center text-slate-500">No se encontraron estudiantes.</td>
              </tr>
              <tr v-else v-for="student in students" :key="student.id" class="border-b border-brand-border hover:bg-white/5 transition-colors">
                <td class="px-6 py-4">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <div class="text-white font-medium">{{ student.user.name }}</div>
                      <div class="text-xs text-slate-500">{{ student.user.email }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 font-mono text-xs text-slate-400">{{ student.enrollment_number }}</td>
                <td class="px-6 py-4">
                  ...
                  <span class="px-2 py-1 rounded-full text-[10px] font-medium uppercase tracking-wider bg-slate-500/10 text-slate-400" v-else>{{ student.status }}</span>
                </td>
        ...
        <!-- Paginación -->
        <div class="p-4 flex items-center justify-between text-sm text-slate-400 border-t border-brand-border" v-if="pagination.last_page > 1">
