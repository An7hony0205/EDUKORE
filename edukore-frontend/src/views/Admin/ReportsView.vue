...
<template>
  <DashboardLayout>
    <div class="h-full flex flex-col space-y-6">
      <div>
        <h1 class="text-2xl font-bold text-white tracking-tight">Reportes y Exportación</h1>
        <p class="text-slate-400 text-sm mt-1">Genera documentos oficiales y exporta datos de la institución.</p>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        
        <!-- Boletín PDF -->
        <div class="bg-brand-surface border border-brand-border p-6 rounded-xl shadow-lg flex flex-col">
          <div class="flex items-center gap-3 mb-4">
            <div class="p-3 bg-red-500/10 rounded-lg text-red-400">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
            </div>
            <div>
              <h2 class="text-lg font-bold text-white">Libreta de Notas (Boletín)</h2>
              <p class="text-xs text-slate-400">Exporta las calificaciones en PDF</p>
            </div>
          </div>
          
          <div class="mt-auto space-y-4">
            <div>
              <label class="block text-xs font-medium text-slate-400 mb-1">ID del Estudiante</label>
              <input v-model="studentId" type="text" class="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-primary-500 focus:border-primary-500" placeholder="Ej: 034477bd-..." />
            </div>
        ...
        <div class="bg-brand-surface border border-brand-border p-6 rounded-xl shadow-lg flex flex-col">
          <div class="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 class="text-lg font-bold text-white">Reporte de Matrículas</h2>
              <p class="text-xs text-slate-400">Exporta la lista de alumnos a Excel/CSV</p>
            </div>
          </div>
