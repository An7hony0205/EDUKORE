  ...
  <DashboardLayout>
    <div class="p-8 space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-bold text-white tracking-tight">Registro de Calificaciones</h2>
          <p class="text-slate-400 mt-1 text-sm">Ingresa las notas y comentarios para esta evaluación</p>
        </div>
        <div class="flex gap-3">
          <button 
            @click="router.back()"
            class="px-4 py-2 text-sm rounded-lg font-medium bg-slate-800/50 text-slate-300 hover:bg-slate-700 transition-colors border border-slate-700 backdrop-blur-sm"
          >
            Volver
          </button>
          <button 
            @click="saveGrades"
            class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-5 rounded-lg transition-colors flex items-center gap-2 shadow-lg shadow-indigo-500/20"
          >
      ...
      <div class="rounded-2xl border bg-slate-900/50 backdrop-blur-xl" style="border-color: rgba(255,255,255,0.08);">
        <div v-if="academicStore.loading" class="p-8 text-center text-slate-400">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full text-left text-sm text-slate-300">
            <thead class="bg-slate-800/40 text-xs uppercase text-slate-400 border-b border-slate-700/50">
              <tr>
              ...
              <tr v-for="student in students" :key="student.id" class="hover:bg-white/[0.02] transition-colors">
                <td class="px-6 py-4 font-medium text-slate-200">
                  <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-indigo-400 border border-slate-700">
                ...
                <td class="px-6 py-4">
                  <input 
                    ...
                    class="w-full bg-slate-950/50 border border-slate-700/80 text-white text-sm rounded-lg focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 block px-3 py-2 transition-all"
                    placeholder="0-20"
                  />
                </td>
                <td class="px-6 py-4">
