  ...
  <DashboardLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <button class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-xl transition-colors">
          + Emitir Nuevo Cobro
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="p-6 rounded-2xl border bg-white/5 border-white/10 backdrop-blur-md">
          <p class="text-sm font-medium text-slate-400">Recaudado (Mes actual)</p>
          <p class="text-3xl font-bold text-emerald-400 mt-2">{{ formatMoney(stats.totalCollected) }}</p>
        </div>
        <div class="p-6 rounded-2xl border bg-white/5 border-white/10 backdrop-blur-md">
          <p class="text-sm font-medium text-slate-400">Cuentas por Cobrar</p>
          <p class="text-3xl font-bold text-yellow-400 mt-2">{{ formatMoney(stats.totalPending) }}</p>
        </div>
        <div class="p-6 rounded-2xl border bg-white/5 border-white/10 backdrop-blur-md">
          <p class="text-sm font-medium text-slate-400">Alumnos Morosos</p>
      ...
      <div class="rounded-2xl border bg-white/5 border-white/10 backdrop-blur-md overflow-hidden">
        ...
        <div v-else-if="!fees.length" class="p-12 text-center text-slate-400">
          No hay registros financieros todavía.
        </div>
        
        <div v-else class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            <thead class="bg-white/5 text-slate-400 uppercase font-semibold text-xs">
              <tr>
            ...
            <tbody class="divide-y divide-white/10 text-slate-300">
              <tr v-for="fee in fees" :key="fee.id" class="hover:bg-white/5 transition-colors">
                <td class="px-6 py-4 font-medium text-white">{{ fee.title }}</td>
                ...
                <td class="px-6 py-4">
                  ...
                  <span v-else class="px-2.5 py-1 text-xs font-bold rounded-md bg-white/10 text-slate-300">{{ fee.status }}</span>
                </td>
              </tr>
            </tbody>
          </table>
