  ...
  <DashboardLayout>
    <div class="p-8 space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-bold text-white tracking-tight">Registro de Asistencia</h2>
          <p class="text-slate-400 mt-1 text-sm">Gestiona la asistencia de los estudiantes del curso</p>
        </div>
        <button 
          @click="router.back()"
          class="px-4 py-2 text-sm rounded-lg font-medium bg-slate-800 text-slate-300 hover:bg-slate-700 transition-colors border border-slate-700"
        >
          Volver
        </button>
      </div>
      ...
      <div class="p-6 rounded-2xl border" style="background: rgba(255,255,255,0.02); border-color: rgba(255,255,255,0.08);">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div class="flex items-center gap-3">
            <label for="attendanceDate" class="text-sm font-semibold text-slate-300">Fecha:</label>
            <input 
              id="attendanceDate"
              type="date" 
              v-model="attendanceDate"
              @change="onDateChange"
              class="bg-slate-800 border border-slate-700 text-white text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block px-3 py-2"
            />
          </div>
          <button 
            @click="saveAttendance"
            class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-5 rounded-lg transition-colors flex items-center gap-2"
          >
        ...
        <div v-if="academicStore.loading" class="text-slate-400 text-sm py-4">
          Cargando...
        </div>
        
        <div v-else-if="students.length === 0" class="text-slate-400 text-sm py-4">
          No hay estudiantes matriculados en este curso.
        </div>
        
        <div v-else class="overflow-x-auto">
          <table class="w-full text-left text-sm text-slate-300">
            <thead class="bg-slate-800/50 text-xs uppercase text-slate-400 border-b border-slate-700">
              <tr>
            ...
            <tbody class="divide-y divide-slate-700/50">
              <tr v-for="student in students" :key="student.id" class="hover:bg-slate-800/30 transition-colors">
                ...
                <td class="px-4 py-4">
                  <div class="flex items-center gap-4">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                      ...
                      <span class="text-slate-300">Tardanza</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                      ...
                      <span class="text-slate-300">Justificado</span>
                    </label>
                  </div>
                </td>
              </tr>
